 ...
 * addLeaf($id, $label, $link, $float=self::ODMENUPOSITION_LEFT, $parent = NULL, $target = self;;ODMENUTARGET_SELF,
 *                                                                                                      $enable = true)
 *                          ajout d'une feuille ou option de menu à l'abre menu global
 * getLeaf($ref)            recherche d'une feuille par sa référence ($id dans addLeaf())
 * getLeafByPath($path = null)
 *                          recherche d'une feuille par son chemin d'accès dans l'arbre
 * deactiveAll()            désactive toute sélection dans le menu
 * active($id)              active une option de menu et son arbre enfant (dropdown)
 * getActive()              retourne l'option de menu active
 * removeLeaf($id)          suppression de la feuille ou option de menu (et enfant : dropdown)
 * clearTree()              vide l'arbre menu
 * updateLeaf($id, $item, $keepDropdown = true)
 ...
 * getModeConstants()
 */
class ODMenuOld extends ODContained
{
    ...
    public function getLeaf($ref)
    {
        ...
        if (array_key_exists($ref, $dataPath)) {
        ...
        if ($path != '') {
            ...
            foreach ($refs as $ref) {
                if ($first) {
                    ...
                    if (array_key_exists('dropdown', $leaf)) {
                        $children   = $leaf['dropdown'];
