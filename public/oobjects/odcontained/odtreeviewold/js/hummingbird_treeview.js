(function($){
	...
	var args = arguments;
	// get treeview options save in data attribute 'treeOptions' if exist
	var defaut = $(this).data('treeOptions');
	// if undefined treeview data attribute 'treeOptions' : get default options
	if (defaut == undefined) { defaut = $.extend( {}, $.fn.hummingbird.defaults, options); }
	// add to treeview options object 'defaut' variable content 'options' on key content 'options' 
	if (options != undefined) { defaut[options] = options; }
	// set variable options with the content of object defaut
	var options = defaut;
	// $.extend( {}, $.fn.hummingbird.defaults, options);
	//initialisation
	if (typeof(methodName) == "undefined" || methodName == "options") {
		if (methodName == "options") {
			// get array of new options
			var arg	= args[1];
			// update options object with the new values
			$.each(arg, function(idx, val){
				options[idx] = val;
			})
		}
		// save in treeview data attribute treeOptions the objet options
		$.fn.hummingbird.updateOption($(this), options);
	...
	$.fn.hummingbird.updateOption = function(tree, options) {
