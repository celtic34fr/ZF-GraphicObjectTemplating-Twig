...
L’événement *change* sera déclenché sur tout changement de valeur de l’objet **ODInput**.

Pour ce qui est de l’événement *keyup*, il sera déclenché à chaque relâchement d’une touche du clavier.
...
| --------------------------- | ------------------------------------------------------------------------------------------------ |
| **getChange()** | Récupération des paramètres de l’évènement *change* sur la zone de saisie. Fait appel à la méthode **getEvent()** pour l’événement change dans la classe mère **OObject**. |
| --------------------------- | ------------------------------------------------------------------------------------------------ |
...
| --------------------------- | ------------------------------------------------------------------------------------------------ |
| **getKeyup()** | Récupération des paramètres de l’évènement *keyup* sur la zone de saisie. Fait appel à la méthode **getEvent()** pour l’événement keyup dans la classe mère **OObject**. |
| --------------------------- | ------------------------------------------------------------------------------------------------ |
...
| | Retourne la valeur faux (**false**) si l’attribut n’est pas trouvé.
| --------------------------- | ------------------------------------------------------------------------------------------------ |
| **enaMask($mask)** | Active l’utilisation du masque de saisie $mask sauvegardé dans l’attribut *mask*. L’attribut *type* est forcé au type **TEXT** en cas d’utilisation de masque. |
| | La méthode retourne l’objet lui-même si tout est vérifié, sinon retourne la valeur faux (**false**). |
| --------------------------- | ------------------------------------------------------------------------------------------------ |
